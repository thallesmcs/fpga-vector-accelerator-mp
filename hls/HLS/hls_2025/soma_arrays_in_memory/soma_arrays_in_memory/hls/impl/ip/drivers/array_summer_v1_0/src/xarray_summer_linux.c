...
typedef struct {
    ...
    XArray_summer_uio_map maps[ MAX_UIO_MAPS ];
} XArray_summer_uio_info;

/***************** Variable Definitions **************************************/
static XArray_summer_uio_info uio_info;
...
static int uio_info_read_name(XArray_summer_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XArray_summer_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XArray_summer_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XArray_summer_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XArray_summer_Initialize(XArray_summer *InstancePtr, const char* InstanceName) {
	XArray_summer_uio_info *InfoPtr = &uio_info;
