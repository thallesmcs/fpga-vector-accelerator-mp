...
entity mult_hw_1600_control_s_axi is
generic (
...
end entity mult_hw_1600_control_s_axi;
...
architecture behave of mult_hw_1600_control_s_axi is
    ...
    constant ADDR_B_HIGH  : INTEGER := 16#3fff#;
    constant ADDR_A_BASE  : INTEGER := 16#4000#;
    constant ADDR_A_HIGH  : INTEGER := 16#5fff#;
    constant ADDR_C_BASE  : INTEGER := 16#6000#;
    constant ADDR_C_HIGH  : INTEGER := 16#7fff#;
    ...
    -- memory signals
    signal int_B_address0      : UNSIGNED(10 downto 0);
    signal int_B_ce0           : STD_LOGIC;
    signal int_B_q0            : UNSIGNED(31 downto 0);
    signal int_B_address1      : UNSIGNED(10 downto 0);
    ...
    end component mult_hw_1600_control_s_axi_ram;
...
-- ----------------------- Instantiation------------------
-- int_B
int_B : mult_hw_1600_control_s_axi_ram
generic map (
     MEM_STYLE  => "auto",
     MEM_TYPE   => "2P",
     BYTE_WIDTH => 8,
...
port map (
     ...
     q1         => int_B_q1);
-- int_A
int_A : mult_hw_1600_control_s_axi_ram
generic map (
     MEM_STYLE  => "auto",
     MEM_TYPE   => "2P",
     BYTE_WIDTH => 8,
...
port map (
     ...
     q1         => int_A_q1);
-- int_C
int_C : mult_hw_1600_control_s_axi_ram
generic map (
     MEM_STYLE  => "auto",
     MEM_TYPE   => "S2P",
     BYTE_WIDTH => 8,
...
-- ----------------------- AXI READ ----------------------
    ...
    RRESP   <= "00";  -- OKAY
    RVALID_t  <= '1' when (rstate = rddata) and (int_B_read = '0') and (int_A_read = '0') and (int_C_read = '0') else '0';
    RVALID    <= RVALID_t;
...
-- ----------------------- Memory logic ------------------
    -- B
    int_B_address0       <= RESIZE(UNSIGNED(B_address0), 11);
    int_B_ce0            <= B_ce0;
    B_q0                 <= STD_LOGIC_VECTOR(RESIZE(int_B_q0, 32));
    int_B_address1       <= raddr(12 downto 2) when ar_hs = '1' else waddr(12 downto 2);
    ...
    -- A
    int_A_address0       <= RESIZE(UNSIGNED(A_address0), 11);
    int_A_ce0            <= A_ce0;
    ...
    -- C
    int_C_address0       <= RESIZE(UNSIGNED(C_address0), 11);
    int_C_ce0            <= C_ce0;
    int_C_be0            <= (others => C_we0);
    int_C_d0             <= RESIZE(UNSIGNED(C_d0), 32);
    int_C_address1       <= raddr(12 downto 2);
...
entity mult_hw_1600_control_s_axi_ram is
    generic (
        MEM_STYLE  : STRING :="auto";
        MEM_TYPE   : STRING :="S2P";
        BYTE_WIDTH : INTEGER :=8;
...
end entity mult_hw_1600_control_s_axi_ram;

architecture behave of mult_hw_1600_control_s_axi_ram is
