...
entity mult_hw_50_control_s_axi is
generic (
...
end entity mult_hw_50_control_s_axi;
...
--          others - reserved
-- 0x1000 ~
-- 0x10ff : Memory 'A' (50 * 32b)
--          Word n : bit [31:0] - A[n]
-- 0x2000 ~
-- 0x20ff : Memory 'B' (50 * 32b)
--          Word n : bit [31:0] - B[n]
-- 0x3000 ~
-- 0x30ff : Memory 'C' (50 * 32b)
--          Word n : bit [31:0] - C[n]
-- (SC = Self Clear, COR = Clear on Read, TOW = Toggle on Write, COH = Clear on Handshake)

architecture behave of mult_hw_50_control_s_axi is
    ...
    constant ADDR_A_HIGH  : INTEGER := 16#10ff#;
    constant ADDR_B_BASE  : INTEGER := 16#2000#;
    constant ADDR_B_HIGH  : INTEGER := 16#20ff#;
    constant ADDR_C_BASE  : INTEGER := 16#3000#;
    constant ADDR_C_HIGH  : INTEGER := 16#30ff#;
    ...
    component mult_hw_50_control_s_axi_ram is
        generic (
    ...
    end component mult_hw_50_control_s_axi_ram;
...
-- ----------------------- Instantiation------------------
-- int_A
int_A : mult_hw_50_control_s_axi_ram
generic map (
     MEM_STYLE  => "auto",
     MEM_TYPE   => "2P",
     BYTE_WIDTH => 8,
...
port map (
     ...
     q1         => int_A_q1);
-- int_B
int_B : mult_hw_50_control_s_axi_ram
generic map (
     MEM_STYLE  => "auto",
     MEM_TYPE   => "2P",
     BYTE_WIDTH => 8,
...
port map (
     ...
     q1         => int_B_q1);
-- int_C
int_C : mult_hw_50_control_s_axi_ram
generic map (
     MEM_STYLE  => "auto",
     MEM_TYPE   => "S2P",
     BYTE_WIDTH => 8,
...
-- ----------------------- Memory logic ------------------
    -- A
    int_A_address0       <= RESIZE(UNSIGNED(A_address0), 6);
    int_A_ce0            <= A_ce0;
    ...
    -- B
    int_B_address0       <= RESIZE(UNSIGNED(B_address0), 6);
    int_B_ce0            <= B_ce0;
    ...
    -- C
    int_C_address0       <= RESIZE(UNSIGNED(C_address0), 6);
    int_C_ce0            <= C_ce0;
    int_C_be0            <= (others => C_we0);
    int_C_d0             <= RESIZE(UNSIGNED(C_d0), 32);
    int_C_address1       <= raddr(7 downto 2);
...
entity mult_hw_50_control_s_axi_ram is
    generic (
        MEM_STYLE  : STRING :="auto";
        MEM_TYPE   : STRING :="S2P";
        BYTE_WIDTH : INTEGER :=8;
...
end entity mult_hw_50_control_s_axi_ram;

architecture behave of mult_hw_50_control_s_axi_ram is
